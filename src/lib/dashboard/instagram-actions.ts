...
export interface ScheduleFit {
  total: number;
  dentro: number;
  /** Días de la semana en que hay propuestas fuera de la franja, con cuántas.
   * Sirve para saber qué mover, no solo cuánto. */
  fueraPorHora: { hora: number; cantidad: number }[];
}
...
export async function getScheduleFit(
  ...
  for (const r of rows) {
    ...
    // getUTCHours() + (-4) la devuelve a la hora local de Santo Domingo.
    const hora = (cuando.getUTCHours() + 20) % 24;
    if (hora >= from && hora < to) dentro++;
    else porHora.set(hora, (porHora.get(hora) ?? 0) + 1);
  }

  return {
    total: interpretadas,
    dentro,
    fueraPorHora: [...porHora.entries()]
      .map(([hora, cantidad]) => ({ hora, cantidad }))
      .sort((a, b) => b.cantidad - a.cantidad || a.hora - b.hora),
  };
}
